...
<div class="w-full px-6 max-w-xl mx-auto">
    ...
    />
    <div class="w-full h-10"></div>
    <button on:click={handleDeposit} disabled={txPending} class={`bg-primary text-white rounded-md font-mono text-xl w-full p-2 hover:shadow-xl hover:shadow-blue-300 transition-all ease-out duration-100`}>
        {#if txPending}
    ...
    <div class="font-mono flex flex-col items-center justify-center mt-2">
        {#if didDeposit}
            <p>{`Success! Deposited ${parseEther(input)} ETH to Base.`}&nbsp;</p>
            <a href={`${BLOCK_EXPLORERS[1]}tx/${txHash}`} class="text-primary">View Mainnet Transaction</a>
            <br>
            <p>Deposits should appear in account in {'<'} 5 min</p>
        {:else}
            {$errorMessage}
        {/if}
    </div>
